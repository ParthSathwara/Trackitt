...
class StockConsumer(AsyncWebsocketConsumer):
    ...
    def addToCeleryBeat(self, stockpicker):
        ...
        if len(task)>0:
            ...
            task = task.first()
            args = json.loads(task.args)# type: ignore
            args = args[0]
            for x in stockpicker:
                if x not in args:
                    args.append(x)
            task.args = json.dumps([args])# type: ignore
            task.save()# type: ignore
        else:
            schedule, created = IntervalSchedule.objects.get_or_create(every=10, period = IntervalSchedule.SECONDS)
            task = PeriodicTask.objects.create(interval = schedule, name='every-10-seconds', task="tracker.tasks.update_stocks", args = json.dumps([stockpicker]))
